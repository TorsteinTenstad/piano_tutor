...
def probability_of_notes(fs, rechording_data, min_key=20, max_key=88):
    spectrum = np.abs(np.fft.fft(rechording_data))
    spectrum = spectrum[:int(len(spectrum)/2)]
    f = (np.arange(len(spectrum))*fs)/len(rechording_data)
...
if __name__ == "__main__":
    ...
    seconds = 0.05  # Duration of rechording
    previously_played_keys = []
    while True:
        rechording_data = sd.rec(int(seconds * fs), samplerate=fs, channels=2)
        sd.wait()  # Wait until rechording is finished
        rechording_data = np.average(rechording_data, axis=1)
        rechording_data = rechording_data/np.max(rechording_data)
        keys, probability = probability_of_notes(fs, rechording_data)
